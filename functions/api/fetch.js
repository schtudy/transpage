...
export async function onRequestGet({ request }) {
  ...
  let res;
  try {
    res = await fetch(u.href, { redirect: 'follow', headers: {
      'user-agent': 'Mozilla/5.0 (compatible; DamdaFetcher/1.0)', accept: 'text/html,application/xhtml+xml' } });
  ...
  return new Response(buf, { headers: {
    'content-type': 'application/octet-stream', 'x-final-url': res.url,
    'x-charset': (ct.match(/charset=([\w-]+)/i) || [])[1] || '', 'cache-control': 'no-store' } });
}
